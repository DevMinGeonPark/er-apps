...
#   ./smoke.sh                              (로컬 dist)
#   ./smoke.sh https://devmingeonpark.github.io/er-apps   (배포본)
set -uo pipefail
cd "$(dirname "$0")"
...
render() { # render <경로> <기대문자열> <설명>
  ...
  ( "$CHROME" --headless=new --user-data-dir="$dir" --disable-gpu --no-first-run \
      ...
      --virtual-time-budget=45000 --dump-dom "$BASE/$1" >/tmp/smoke.html 2>/dev/null ) & local ch=$!
...
[ -z "${1:-}" ] && render "death/_pngtest.html?name=$N" "PNGRESULT [0-9]" "진단서 PNG 래스터화(html2canvas)"
