// 루미아 신용정보원 — (유저 × 실험체) 단위 신용평가. dak.gg 기반, 의존성 없음 (Node 18+)
...
const getCharacters = async () => (await dakJson('/data/characters?hl=ko', META_TTL)).characters
  .map(c => ({ id: c.id, key: c.key, name: c.name, imageUrl: 'https:' + c.imageUrl }));

// ---------- 조회 이력 (신용조회는 원래 남이 하는 것) ----------
const INQ_FILE = path.join(__dirname, 'data', 'inquiries.json');
const INQ_WINDOW = 30 * 86400 * 1000;
let inquiries = {};
try { inquiries = JSON.parse(fs.readFileSync(INQ_FILE, 'utf8')); } catch { inquiries = {}; }
let inqDirty = false;
// ponytail: 5초 주기 일괄 flush. 조회량 늘면 SQLite로.
setInterval(() => {
  if (!inqDirty) return;
  inqDirty = false;
  fs.writeFile(INQ_FILE, JSON.stringify(inquiries), () => {});
}, 5000).unref();

function recordInquiry(name, characterId) {
  const key = `${name.toLowerCase()}|${characterId}`;
  const now = Date.now();
  const list = (inquiries[key] || []).filter(t => now - t < INQ_WINDOW);
  list.push(now);
...
function readInquiry(name, characterId) {
  const key = `${name.toLowerCase()}|${characterId}`;
  const now = Date.now();
  return (inquiries[key] || []).filter(t => now - t < INQ_WINDOW).length;
}
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (url.pathname === '/api/report') {
      ...
      const d = await buildDossier(name);
      if (!d.seasonsPlayed || !(d.overall.play > 0)) {
        return sendJson(res, 404, { error: `'${d.player.name}'님의 거래 이력이 없습니다. 전적이 있는 닉네임으로 조회하십시오.` });
      }
    ...
    // 개별 실험체 신용조사서 (조회 이력이 남는다)
    if (url.pathname === '/api/credit') {
      const name = (url.searchParams.get('name') || '').trim();
      const characterId = parseInt(url.searchParams.get('characterId'), 10);
      if (!name || !characterId) return sendJson(res, 400, { error: '닉네임과 실험체를 지정하세요.' });
      const d = await buildDossier(name);
      const ent = d.chars.get(characterId);
      const character = d.charById.get(characterId);
      if (!character) return sendJson(res, 400, { error: '알 수 없는 실험체입니다.' });
      if (!ent || !ent.total.play) {
        return sendJson(res, 404, { error: `'${d.player.name}'님의 ${character.name} 거래 이력이 없습니다.` });
      }
    ...
    if (url.pathname.startsWith('/img/char/')) {
      ...
      const c = (await getCharacters()).find(x => x.key === key);
    ...
    const code = e.status === 404 ? 404 : (e.status === 400 ? 400 : 500);
    const msg = e.status === 404 ? '조회 대상을 찾을 수 없습니다. 닉네임을 확인해주세요.' : String(e.message || e);
    sendJson(res, code, { error: msg });
  }
});

if (require.main === module) {
  server.listen(PORT, () => console.log(`er-credit listening on http://localhost:${PORT}`));
}
module.exports = { evaluate, MODEL, GRADES, zero, norm, clamp01 };
